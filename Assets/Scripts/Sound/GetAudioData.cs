...
public class GetAudioData : MonoBehaviour
{
    ...
    private float CalculateSpectralCentroid( float[] fft )
    {
        ...
        // TODO
        float hzPerBin = AudioSettings.outputSampleRate * 0.5f / fft.Length;
    ...
    private void Float2Complex( float[] input, Complex[] result, float[] window )
    {
        ...
        {
            result[i] = new Complex(input[i] * window[i], 0);
        }
    }
}
